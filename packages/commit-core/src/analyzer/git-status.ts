...
function detectNestedRepo(cwd: string, scope: string): string | undefined {
  ...
  // 'packages-foo' -> 'packages-foo'
  const baseDir =
    normalizedScope.split("-").length > 0
      ? normalizedScope
      : scope.split("/").filter((p) => !p.includes("*"))[0];
  if (!baseDir) {
    return undefined;
  }
 ...
 */
async function getNestedRepoStatus(
  rootCwd: string,
  nestedPath: string,
): Promise<GitStatus> {
  ...
  return {
    staged: status.staged.filter((f) => !shouldIgnoreFile(f)).map(prefixPath),
    unstaged: [...status.modified, ...status.deleted]
      .filter((f) => !status.staged.includes(f))
      .filter((f) => !shouldIgnoreFile(f))
      .map(prefixPath),
