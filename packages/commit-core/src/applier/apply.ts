...
import type { CommitPlan, ApplyResult, CommitGroup } from '@kb-labs/commit-contracts';
...
async function applyCommit(cwd: string, commit: CommitGroup): Promise<string> {
  ...
  if (repos.length > 1) {
    throw new Error('Commit spans multiple repositories. Split into separate commits.');
  }
...
function groupFilesByRepo(
  ...
  const filesByRepo = new Map<string, { relativePath: string; originalPath: string }[]>();
 ...
 */
export function formatCommitMessage(commit: CommitGroup, options?: { includeFooter?: boolean }): string {
